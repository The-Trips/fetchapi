...
create table if not exists users (
  u_id int primary key references priv_users(u_id) on delete cascade,
...
create table if not exists scenes (
  ...
  scene_uid uuid not null unique default gen_random_uuid(),
  name varchar,
  description text,
  image_url varchar,
  date_created timestamp not null,
...
create table if not exists threads (
  ...
  t_uid uuid not null unique default gen_random_uuid(),
  title varchar,
  text text,
  date_created timestamp not null,
  u_id int not null references users(u_id) on delete cascade,
...
create table if not exists replies (
  ...
  rep_uid uuid not null unique default gen_random_uuid(),
  text text,
  level int,
  thread_id int not null references threads(t_id) on delete cascade,
  
  -- CHANGED: Removed "NOT NULL" so you can create top-level replies
  parent_rep_id int references replies(rep_id) on delete cascade,
...
create table if not exists user_moods (
  ...
  -- The fixed constraint allowing multiple moods per album
